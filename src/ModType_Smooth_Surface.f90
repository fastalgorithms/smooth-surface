...
!
    ...
    integer ( kind = 8 ) n_dummy_targ   !total number of dummy targets
...
!Edges and Boundary is use to compute the line integral in the non-FMM case (beyond that line integral, the kernel is 1/r and erf=1)
    integer ( kind = 8 ), allocatable :: Edges(:,:)             !Set of 3 integers with the location of the points Points(1:3,:) for each edge in the skeleton (set of quadratic triangles)
    ...
    integer ( kind = 8 ), allocatable :: Boundary(:,:)          ! Set of 3 Edges for each triangle in the skeleton:  Boundary(n_edge=1 to 3,num of triangle)
!!
    ...
    integer ( kind = 8 ) n_order_sf                             !number of nodes per smooth triangle (45 or 78)
