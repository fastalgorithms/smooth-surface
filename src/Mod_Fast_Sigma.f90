...
!!This routine initialize the tree to allow a fast sigma evaluator
    ...
    integer ( kind = 8 ) n_leaf_boxes,count1
...
!! Use speed_flag=0 will be only interesting to debug. Compute the value by brute force, not using the tree structure
!! in that case, the other flag is not relevant
!!
...
!! Use adapt_flag=2 for HUGE adaptivity requirement
    ...
    integer ( kind = 8 ), intent(in) :: n_targ,adapt_flag               !! Number of targets and flags to specify adaptivity and speed
                                                                        !! adapt_flag=0 means constant sigma (no adaptivity at all)
                                                                        !! adapt_flag=1 means constant alfa (adaptiviti)
                                                                        !! adapt_flag=2 means recursive sigma definition (HUGE adaptivity and a bit slower)
    real ( kind = 8 ), intent(in) :: targ_vect(3,n_targ)                !! vector with the targets
    real ( kind = 8 ), intent(out) :: sgma(n_targ),sgma_x(n_targ),sgma_y(n_targ),sgma_z(n_targ) !! output sigma and gradient of sigma

        !! this function calls the corresponding subroutine with the appropriate sigma generator for the selected flats
        if (adapt_flag==0) then
            call function_eval_sigma_v0(targ_vect,n_targ,FSS_1,sgma,sgma_x,sgma_y,sgma_z)   !! sigma will be constant across the geometry
...
!! This subroutine computes sgma with no adaptivity at all (constant sgma and derivatives=0)
    ...
    !List of calling arguments
    type ( Fast_Sigma_stuff ), pointer ::  FSS_1
    integer ( kind = 8 ), intent(in) :: n_targ
