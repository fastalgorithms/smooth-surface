function [tri_flat_tot,F_x,F_y,F_z,COL,Err_L2,Err_L1,Err_Lsup]=load_flattri_smooth_geometry(ntri,norder,npoints,srcvals,N)
    u=linspace(0,1,N+1);
    [U,V]=meshgrid(u);  
    icount1=1;
    icount2=1;
...
%     return
    ...
    CCx_tail=sum(abs(CCx(end-(norder+1)+1:end,:)).^2);
    CCx_body=sum(abs(CCx(1:end-(norder+1),:)).^2);

    CCy_tail=sum(abs(CCy(end-(norder+1)+1:end,:)).^2);
    CCy_body=sum(abs(CCy(1:end-(norder+1),:)).^2);

    CCz_tail=sum(abs(CCz(end-(norder+1)+1:end,:)).^2);
    CCz_body=sum(abs(CCz(1:end-(norder+1),:)).^2);

    CC_tail=CCx_tail+CCy_tail+CCz_tail;
    CC_body=CCx_body+CCy_body+CCz_body;
    error_coefs_L2=sqrt(CC_tail./CC_body);
    

    
    
    CCx_tail=sum(abs(CCx(end-(norder+1)+1:end,:)));
    CCx_body=sum(abs(CCx(1:end-(norder+1),:)));

    CCy_tail=sum(abs(CCy(end-(norder+1)+1:end,:)));
    CCy_body=sum(abs(CCy(1:end-(norder+1),:)));

    CCz_tail=sum(abs(CCz(end-(norder+1)+1:end,:)));
    CCz_body=sum(abs(CCz(1:end-(norder+1),:)));

    CC_tail=CCx_tail+CCy_tail+CCz_tail;
    CC_body=CCx_body+CCy_body+CCz_body;
    error_coefs_L1=(CC_tail./CC_body);

    
    
    
    CCx_tail=max(abs(CCx(end-(norder+1)+1:end,:)));
    CCx_body=max(abs(CCx(1:end-(norder+1),:)));

    CCy_tail=max(abs(CCy(end-(norder+1)+1:end,:)));
    CCy_body=max(abs(CCy(1:end-(norder+1),:)));

    CCz_tail=max(abs(CCz(end-(norder+1)+1:end,:)));
    CCz_body=max(abs(CCz(1:end-(norder+1),:)));

    CC_tail=CCx_tail+CCy_tail+CCz_tail;
    CC_body=CCx_body+CCy_body+CCz_body;
    error_coefs_Lsup=(CC_tail./CC_body);
...
%    col=error_coefs;
    ...
    aux=ones(N^2*3,1);
    Err_L2=aux*error_coefs_L2;
    Err_L2=Err_L2(:);

    aux=ones(N^2*3,1);
    Err_L1=aux*error_coefs_L1;
    Err_L1=Err_L1(:);

    aux=ones(N^2*3,1);
    Err_Lsup=aux*error_coefs_Lsup;
    Err_Lsup=Err_Lsup(:);
