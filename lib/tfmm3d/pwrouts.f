...
c     Temp variables
      ...
      do nl=1,nlambs
         ...
         do mth = 0,numtets(nl)-1
            ...
            sgn = -sgn
            do nm = mth,nterms,2
               ztmp1 = ztmp1 + rlsc(nl,nm,mth)*mpole(nm,mth)
            enddo

            do nm=mth+1,nterms,2
               ztmp2 = ztmp2 + rlsc(nl,nm,mth)*mpole(nm,mth)
            enddo
...
c -----------------------------------------------------------------
      subroutine exptolocalup(local,nterms,rlambs,whts,nlambs,numtets,
     1                         nthmax,nexptot,lexp1f,scale)
c-----------------------------------------------------------------
...
c        Add contributions to local expansion
         do nm=0,nterms,2
            ...
            do mth=0,mmax
               ncurrent = ntot+mth
               local(nm,mth) = local(nm,mth)+rmul*lexp1f(ncurrent)
            enddo
         enddo
         do nm=1,nterms,2
            mmax = numtets(nl) - 1
...
c            print *, nm,mth
            ...
            local(nm,mth) = local(nm,mth)*zeye(mth)*rscale
...
c-----------------------------------------------------------------      
      ...
      subroutine exptolocaldown(local,nterms,rlambs,whts,nlambs,numtets,
     1                         nthmax,nexptot,lexp1f,scale)
c-----------------------------------------------------------------
c     This sburoutine converts the Fourier representation of
c     an exponential moment function into a local
...
c------------------------------------------------------------------
     ...
     1                         nthmax,nexptot,lexp1f,lexp2f,scale)
c-----------------------------------------------------------------
c     INPUT arguments
c     nterms           in: integer
c                      Order of local expansion
